...
const logger = getLogger('email-strategy');
...
export class EmailStrategy implements NotificationStrategy {
  ...
  constructor(settings: Settings) {
    this.provider = settings.NOTIFICATION_EMAIL_PROVIDER;
  ...
  async send(notification: Record<string, unknown>): Promise<NotificationResult> {
    ...
    logWithContext(logger, 'INFO', 'Email Mock 발송 완료', {
      channel: 'email',
    ...
    return {
      ...
      channel: 'email',
      provider: this.provider,
      message: `Mock email sent (delay: ${delayMs}ms)`,
      duration_ms: durationMs,
    };
  }
