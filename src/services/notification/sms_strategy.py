...
class SmsStrategy(NotificationStrategy):
    """SMS Mock 발송 Strategy.

    MOCK_DELAY_MIN_MS ~ MOCK_DELAY_MAX_MS 범위 랜덤 딜레이 후 성공 반환.
    provider는 NOTIFICATION_SMS_PROVIDER 환경 변수에서 결정.
    """

    def __init__(self, settings: Settings) -> None:
        self._provider = settings.NOTIFICATION_SMS_PROVIDER
    ...
    async def send(self, notification: dict[str, object]) -> NotificationResult:
        ...
        return NotificationResult(
            ...
            channel="sms",
            provider=self._provider,
            message=f"Mock SMS sent (delay: {delay_ms}ms)",
            duration_ms=round(duration_ms, 2),
        )
